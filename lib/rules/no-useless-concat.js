        ...
        return {
            BinaryExpression(node) {
                ...
                if (astUtils.isStringLiteral(left) &&
                    ...
                    const value = leftRaw + rigthRaw
                    console.log({value: left.value, raw: left.raw})
                    console.log({value: right.value, raw: right.raw})
                    console.log(left.raw + right.raw)
                    console.log(left.value + right.value)
                    console.log('\'' + left.value + right.value + '\'')
                    //console.log(right.quasis[0].value)

                    //if (left.type == 'TemplateLiteral' && right.type != 'TemplateLiteral' && leftRaw.search(/[`]/g) != -1) {
                    //  console.log("exit ``")
                    //  return;
                    //}
                    ...
                    context.report({
                        ...
                        fix: function(fixer) {
                          // old: '\'' + leftRaw + rigthRaw + '\''
                          return fixer.replaceText(node, JSON.stringify(leftRaw + rigthRaw)) // `"${value}"`
                        }
                    });
                }
            }
